...
class testbed:
	...
	def __init__(self, mcLearning = False, sarsaLearning = False, qLearning = False, randomSpwan=False, p1=0.8, p2=0.1, epsilon = 0.1, gamma = 0.9, alpha = 0.1,plotresults = True):
		...
		#Epsilon, Gamma, Alpha
		if self.mcLearning: 
			self.policy = mc.monteCarlo(epsilon, gamma, alpha)
		...
		if self.sarsaLearning:
			self.policy = sarsa.sarsaLearning(epsilon, gamma, alpha)
		...
		if self.qLearning: 
			self.policy = q.qlearning(epsilon, gamma, alpha)
			...
			for ep in range(1,episodes+1):
				...
				while True:
					...
					if self.gridWorld.finished or agent.moves> steps:
						#Reset goal state
						if self.plotter: self.plotter.LogResults(agent.moves,ep)
